...
def get_enviro_load_list_excel(
    ...
    all_panels_sheet = create_enviro_load_list_excel(
    ...
    for panel_name, panel_data in panels_data.items():
        ...
        panel_sheet = create_enviro_load_list_excel(
...
def create_enviro_load_list_excel(electrical_load_list_data, load_list_output_sheet):
    """
    ...
    for index, data in enumerate(electrical_load_list_data):
        ...
        load_list_output_sheet.cell(row=row, column=6, value=data.get("kva"))
        load_list_output_sheet.cell(row=row, column=7, value=data.get("starter_type"))
        load_list_output_sheet.cell(
            row=row, column=8, value=f"{data.get('supply_voltage')} VAC"
        )
        load_list_output_sheet.cell(row=row, column=9, value=data.get("phase"))
        load_list_output_sheet.cell(
            row=row, column=10, value=data.get("motor_rated_current")
        )
        load_list_output_sheet.cell(
            row=row, column=11, value=data.get("control_scheme")
        )
